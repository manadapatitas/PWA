...
function renderizarHistorialClinicoPaciente(rutTutor, nombreMascota) {
  ...
  [...atencionesMascota].reverse().forEach(c => {
    ...
    card.innerHTML = `
      ...
      ${c.receta ? '<button class="btn-imprimir-receta" style="margin-top:8px;font-size:0.8rem;padding:4px 10px;background:#2e7d32;color:#fff;border:none;border-radius:4px;cursor:pointer;">🖨️ Imprimir receta</button>' : ''}
    `;
...
function imprimirReceta(atencion) {
  ...
  const html = `<!DOCTYPE html><html lang="es"><head><meta charset="UTF-8">
    ...
    <style>
      ...
      .encabezado { text-align: center; border-bottom: 2px solid #2e7d32; padding-bottom: 10px; margin-bottom: 20px; }
      .encabezado h1 { margin: 0; color: #2e7d32; font-size: 1.4rem; }
      .encabezado p { margin: 2px 0; font-size: 0.8rem; color: #666; }
      ...
      .firma .linea { border-top: 1px solid #333; width: 220px; margin: 0 auto 5px; }
    </style></head><body>
    <div class="encabezado"><h1>🐾 Manada Patitas</h1><p>Receta Médica Veterinaria</p></div>
    ...
    <div class="firma"><div class="linea"></div><div>${atencion.atendido_por || 'Médico Veterinario'}</div></div>
    </body></html>`;
