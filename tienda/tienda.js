...
async function iniciarCheckout(event) {
  ...
  if (btn) { btn.disabled = true; btn.innerText = 'Conectando con Webpay...'; }
  ...
  try {
    const json = await llamarBackend('iniciarPagoWebpay', {
      items: carrito.map(i => ({ sku: i.sku, nombre: i.nombre, precio: i.precio, cantidad: i.cantidad })),
      cliente
    });

    // Webpay exige una redirección real vía POST (no fetch/AJAX ni iframe).
    ...
    if (btn) { btn.disabled = false; btn.innerText = 'Pagar con Webpay'; }
  }
}
...
async function cargarHorariosPublicos() {
  ...
  try {
    ...
    const horas = ["09:00", "09:30", "10:00", "10:30", "11:00", "11:30", "12:00", "12:30",
                   "13:00", "13:30", "15:00", "15:30", "16:00", "16:30", "17:00", "17:30", "18:00"];
